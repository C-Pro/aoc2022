...
coords = [coord for coord in [(x, y) for x in range(
    len(M[0])) for y in range(len(M))] if H(coord) == 0]
coords.sort(key=lambda x: V(x))
minPath = 500
...
for S in coords:
    ...
    if paths[E[1]][E[0]] < 189374891234791238734981:
        print(paths[E[1]][E[0]])
        break
